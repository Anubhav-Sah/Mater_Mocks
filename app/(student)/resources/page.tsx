...
import { Resource } from "@/types/resource";
import { toast } from "sonner";
import { Loader2, Download, Eye, FileText, Video, Link as LinkIcon, BookOpen, Database } from "lucide-react";
...
export default function StudentResourcesVaultPage() {
  ...
  // ACTION 1: Clean Inline Browser Preview (Bypasses double token consumption)
  const handleViewInBrowser = async (resourceId: string, title: string) => {
    try {
      toast.loading("Securing access link...", { id: resourceId });
      const response = await resourceService.download(resourceId);
      
      if (response.success && response.data?.downloadUrl) {
        toast.success(`Opening ${title}`, { id: resourceId });
        
        // Open the signed token URL directly in a new tab
        window.open(response.data.downloadUrl, "_blank");
      } else {
        toast.error(response.message || "Failed to generate preview link", { id: resourceId });
      }
    } catch (error: any) {
      toast.error("Unable to securely stream preview.", { id: resourceId });
    }
  };

  // ACTION 2: Direct Single-Use Download Execution
  const handleDownloadForce = async (resourceId: string, title: string) => {
    try {
      toast.loading("Requesting file access...", { id: resourceId });
      const response = await resourceService.download(resourceId);
      
      if (response.success && response.data?.downloadUrl) {
        toast.success(`Downloading ${title}`, { id: resourceId });
        
        // Force a clean browser download window route
        ...
        toast.error(response.message || "Download permission denied", { id: resourceId });
      }
    } catch (error: any) {
      toast.error("File download pipeline connection failed.", { id: resourceId });
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {resources.map((item) => (
                ...
                >
                  ...
                  {/* Dual Functional UI Buttons Split Grid */}
                  <div className="grid grid-cols-2 gap-2 pt-4 mt-5 border-t border-slate-50">
                    <button
                      onClick={() => handleViewInBrowser(item._id!, item.title)}
                      className="flex items-center justify-center gap-1.5 py-2.5 bg-slate-50 hover:bg-slate-900 hover:text-white text-slate-700 rounded-xl transition-all text-[11px] font-bold uppercase tracking-wider border border-slate-100 shadow-sm"
                    >
                      <Eye className="w-3.5 h-3.5" /> View
                    </button>
                    
                    <button
                      onClick={() => handleDownloadForce(item._id!, item.title)}
                      className="flex items-center justify-center gap-1.5 py-2.5 bg-slate-100 hover:bg-[#D00113] hover:text-white text-slate-800 rounded-xl transition-all text-[11px] font-bold uppercase tracking-wider shadow-sm"
                    >
