...
import { TestAttempt } from "@/types/attempt";
import { toast } from "sonner";
import { Loader2, CheckCircle, Clock, Target, Trophy, HelpCircle } from "lucide-react";
import { formatCurrency } from "@/lib/utils";
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      {attempt.answers && attempt.answers.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mt-8">
          ...
          <div className="divide-y divide-slate-100">
            {attempt.answers.map((ans, idx) => {
              const questionData = ans.question as any;
              
              // Dynamically find correct option fallback strings matching schemas safely
              const correctOptionText = questionData?.options?.find(
                (opt: any) => opt.isCorrect || opt._id === questionData?.correctOption
              )?.text || "Correct option details unavailable";
                  ...
                  <div className="flex gap-4">
                    <div className="shrink-0 flex flex-col items-center gap-2">
                      ...
                      {ans.isCorrect ? (
                        ...
                        <div className="w-5 h-5 rounded-full bg-red-100 text-red-500 flex items-center justify-center text-[10px] font-black">X</div>
                      )}
                    </div>
                    <div className="flex-1 space-y-3">
                      <div className="text-sm font-medium text-slate-800" dangerouslySetInnerHTML={{ __html: questionData?.questionText || "Question text not available" }} />
                      
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                        <div className={`p-3 rounded-lg border ${ans.isCorrect ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-800'}`}>
                          <span className="font-bold block mb-1">Your Answer:</span>
                          {ans.selectedOption ? (
                            <span dangerouslySetInnerHTML={{ __html: (ans.selectedOption as any)?.text || "Option text missing" }} />
                          ) : (
                            <span className="italic opacity-60">Unanswered / Skipped</span>
                          )}
                        </div>
                        ...
                        {!ans.isCorrect && (
                          <div className="p-3 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-800">
                            <span className="font-bold block mb-1">Correct Answer:</span>
                            <span dangerouslySetInnerHTML={{ __html: correctOptionText }} />
                          </div>
                        )}
                      </div>

                      {/* Explicit Solution Explanations Panel UI block fallback */}
                      {questionData?.explanation && (
                        <div className="bg-slate-50 rounded-xl p-3 border border-slate-150 mt-2 text-xs text-slate-600 space-y-1">
                          <span className="font-black text-slate-700 uppercase text-[9px] tracking-wider flex items-center gap-1">
                            <HelpCircle className="w-3 h-3 text-slate-400" /> Solution Logic & Explanation:
                          </span>
