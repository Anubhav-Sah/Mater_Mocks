    ...
    const fetchAttempts = async () => {
      try {
        ...
        if (response && response.success) {
          // Robust fallback checks for nested data properties: response.data.data vs response.data
          const targetedArray = response.data?.data || response.data;
          
          if (Array.isArray(targetedArray)) {
            setAttempts(targetedArray);
          } else {
            console.warn("API succeeded but payload structural array format was not resolved:", response);
            setAttempts([]);
          }
        } else {
          console.error("API error response state encountered:", response?.message || "Unknown server validation flag");
          setAttempts([]);
        }
      } catch (error: any) {
        // Unrolls hidden error details instead of displaying an empty object "{}"
        console.error("Detailed Exception Capture:", {
          message: error?.message || "Fallback validation failure",
          status: error?.response?.status,
          backendData: error?.response?.data,
          rawError: error
        });
        
        toast.error(error?.response?.data?.message || error?.message || "Failed to sync performance metrics.");
        setAttempts([]); 
      } finally {
        setIsLoading(false);
      }
            ...
            <table className="w-full text-left text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-600">
                {attempts.map((attempt) => {
                  ...
                  return (
                    <tr key={attempt._id || Math.random().toString()} className="hover:bg-slate-50/60 transition-colors group">
                      <td className="py-4 px-6">
                        <div className="flex flex-col">
                          <span className="font-bold text-slate-900 group-hover:text-[#D00113] transition-colors line-clamp-1">
                            {(attempt.test as any)?.title || "Unknown Test"}
